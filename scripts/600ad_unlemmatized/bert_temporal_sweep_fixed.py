...
from transformers import BertTokenizer, BertConfig, BertForSequenceClassification, DataCollatorWithPadding, get_linear_schedule_with_warmup
...
def train_one_model(train_texts, train_labels, val_texts, val_labels, cfg, seed):
    ...
    config.num_labels = 2
    model = BertForSequenceClassification.from_pretrained(cfg['model_name'], config=config)
    model.to(device)
